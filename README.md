...
Tested: Includes RSpec tests for controller and service logic.
...
Database: MySQL (gem included, but not used for application data storage in this proxy)
...
MySQL Server (installation required for mysql2 gem, but no database setup/migrations are needed for the proxy's core functionality)
...
(Note: The `mysql2` gem requires MySQL client libraries to be installed on your system.)

Set up Environment Variables:
Create a file named `.env` in the root of your project directory. This file will store sensitive credentials for development. Do NOT commit this file to version control.
...
Get CoinGate Sandbox API Credentials:

Sign up for a CoinGate Sandbox account at https://sandbox.coingate.com/.

Log in to your sandbox account.

Look for a menu item or section related to API or Developer.

Within the API section, look for Apps or API Applications.

Click on "+ New App" or a similar button to create new API credentials.
...
Although the proxy doesn't use a database for application data, the Rails framework might expect a database configuration. Run the database creation command:

```bash
rails db:create
```

(Note: No tables or migrations are needed for this proxy application's data storage.)
...
The server will typically start on `http://localhost:3000` in the development environment.
...
(Note: The proxy will override the `callback_url` sent to CoinGate with its own callback endpoint URL.)

Testing Callbacks Locally (Requires Tunneling):
To test the callback endpoint (`/api/v1/orders/callback`), CoinGate's sandbox servers need to be able to reach your local machine. You'll need a tunneling service like ngrok.

Download and run ngrok: `./ngrok http 3000`

ngrok will provide a public URL (e.g., `https://RANDOM_SUBDOMAIN.ngrok-free.app`).

When creating a new order via your proxy, ensure the `callback_url` sent to CoinGate is the ngrok URL pointing to your callback endpoint (e.g., `https://RANDOM_SUBDOMAIN.ngrok-free.app/api/v1/orders/callback`). You can achieve this by configuring `config.action_controller.default_url_options` in `config/environments/development.rb` to use the ngrok host and protocol.

After creating the order, go to the `payment_url` provided in the response, and use the buttons on the CoinGate sandbox invoice page to simulate payment status changes. This will trigger callbacks to your ngrok URL.
...
Circuit Breaker: Add a circuit breaker pattern for calls to CoinGate to handle temporary API outages gracefully.
